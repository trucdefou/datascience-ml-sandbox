...
from modules.enoch_list import enoch_list, delete_from_list, handle_delete
from modules.enoch_reminder import enoch_reminder, delete_from_reminder, handle_delete_reminder, load_enoch_reminder
from static.const import get_add_to_list, get_add_to_reminder
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup
from modules.enoch_list import load_enoch_list
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "1":
        ...
        await enoch_list(update, context)
    elif query.data == "3":
        ...
        await enoch_reminder(update, context)
    elif query.data == "6":
        await context.bot.send_message(
            chat_id=update.effective_chat.id,  # Correctly fetches the chat ID
    ...
    for idx, value in enumerate(enochlist):
        if query.data == f"delete_{idx}":
            await handle_delete(update, context)
    for idx, value in enumerate(enochreminder):
        if query.data == f"delete_{idx}":
