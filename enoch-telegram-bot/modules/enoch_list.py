...
def load_enoch_list():
    try:
        with open("enoch_list.json", "r") as file:
            return json.load(file)
    except FileNotFoundError:
        return []

        
def save_enoch_list(list):
    with open("enoch_list.json", "w") as file:
...
async def handle_add_to_list(update: Update, context: CallbackContext):
    ...
    try:
        ...
        enoch_list = load_enoch_list()
        enoch_list.append(user_input)
        save_enoch_list(enoch_list)
...
async def enoch_list(update: Update, context: CallbackContext):
    enochlist = load_enoch_list()
    message = "Lista de cosas:\n"
    if enochlist:
        for element in enochlist:
            message += f"- {element}\n"
    else:
...
async def delete_from_list(update: Update, context: CallbackContext):
    ...
    buttons = [
        ...
        for idx, value in enumerate(enoch_list)
    ]
...
async def handle_delete(update: Update, context: CallbackContext):
    ...
    if data.startswith("delete_"):
        ...
        enochlist = load_enoch_list()
        if item_to_delete < len(enochlist):
            del enochlist[item_to_delete]
            save_enoch_list(enochlist)
