...
def graph_boxplot(df, columns, num_columns=3, figsize=(14, 10), title="Boxplot"):
    # Filter valid columns (numeric and non-empty)
    valid_columns = [col for col in columns if pd.api.types.is_numeric_dtype(df[col]) and not df[col].dropna().empty]

    if not valid_columns:
        raise ValueError("No valid numeric columns to plot.")

    num_rows = int(np.ceil(len(valid_columns) / num_columns))
    ...
    # Plot boxplots for each valid column
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.98])   # Ajusta la distribución del layout
    plt.show()
...
def graph_confusion_matrixes(
    ...
    column_num=3,
    figsize=(10, 8)):
    row_num = int(np.ceil(len(conf_matrixes) / column_num))  # Ajuste para redondear filas
    
    # Capturar fig y axes
    fig, axes = plt.subplots(nrows=row_num, ncols=column_num, figsize=figsize)

    # Convertir axes a un arreglo plano (para manejar casos de 1 fila/columna)
    axes = np.atleast_1d(axes).flatten()

    for idx, matrix_confusion in enumerate(conf_matrixes):
        ax = axes[idx]
        ...
        if confusion_matrix_names is not None:
            ax.set_title(f"Matriz de Confusión {confusion_matrix_names[idx]}")
        else:
            ax.set_title("Matriz de Confusión")

    # Eliminar ejes no utilizados
    for idx in range(len(conf_matrixes), len(axes)):
        fig.delaxes(axes[idx])

    plt.tight_layout()
    plt.show()

def graph_pie(df, column_names, num_columns=3, figsize=(14, 10)):
    row_num = int(len(column_names) / num_columns)
    left = len(column_names) % num_columns
