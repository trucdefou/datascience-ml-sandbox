...
def graph_confusion_matrixes(
    ...
    if row_num == 1 and column_num == 1:
        axes = [[axes]]  # Convertir a una matriz 2D
    elif row_num == 1:
        axes = [axes]  # Convertir a una matriz 2D con una fila
    elif column_num == 1:
        axes = [[ax] for ax in axes]  # Convertir a una matriz 2D con una columna
...
def graph_temporal_series(
    ...
    n_rows = int(len(columns) / n_columns)
    remain = len(columns) % n_columns

    if remain > 0:
        n_rows += 1

    _, axes = plt.subplots(nrows=n_rows, ncols=n_columns, figsize=figsize)
    ...
    for column in columns:
        ...
        ax.plot(df[column_x], df[column])

        ax.set_title(f"Serie temporal {column}")
        ax.set_xlabel(column_x)
        ax.set_ylabel(column)

        j_actual += 1

        if j_actual >= n_columns:
            i_actual += 1
            j_actual = 0
